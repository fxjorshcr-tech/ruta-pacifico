    ...
    <form onSubmit={handleSubmit} className="space-y-10">
      ...
      <section className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div>
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2 text-xs font-semibold text-foreground/50">
                    {vehicleTierCopy(v.key, locale).paxLabel}
                  </div>
          ...
          {vehicleTooSmall && (
            ...
            >
              {groupTooLarge
                ? t.groupTooLarge(totalPax)
                : t.vehicleTooSmall(selectedVehicle.name, selectedVehicle.maxPax, totalPax)}
            </p>
          )}
        </div>
        ...
        <div className="mt-8">
          ...
          <p className="mt-3 text-xs text-foreground/50">{t.leadTimeHint}</p>
          {(showDateTimeError || dateTooSoon) && (
            <p
              ref={dateTimeErrorRef}
              className="mt-3 scroll-mt-24 text-xs font-medium text-red-600"
            >
              {dateTooSoon ? leadTimeMessage(locale) : t.chooseDateTime}
            </p>
          )}
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="flex items-center gap-4 rounded-2xl border-2 border-black/5 bg-white p-5 transition hover:border-sunset-orange/40 hover:shadow-md">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-foreground/50">{t.adultsHint}</div>
              </div>
            ...
            <div className="flex items-center gap-4 rounded-2xl border-2 border-black/5 bg-white p-5 transition hover:border-sunset-orange/40 hover:shadow-md">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-xs text-foreground/50">{t.childrenHint}</div>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
        ...
        {isAirportPickup && (
          <div className="mt-6">
            ...
            />
            <p className="mt-1 text-xs text-foreground/40">{t.flightHint}</p>
          </div>
        )}
      ...
      {added ? (
        <div ref={successRef} className="rounded-3xl border border-green-200 bg-green-50 p-6 text-center sm:p-8">
          ...
          <h3 className="mt-4 text-lg font-bold text-foreground">{t.added}</h3>
          <p className="mt-2 text-sm text-foreground/60">
            {t.addedTo(route.origen, route.destino)}
          </p>
          <div className="mt-6 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <button
              type="button"
              onClick={() => router.push(localePath(locale, "/private-shuttle"))}
            ...
            <button
              type="button"
              onClick={() => router.push(localePath(locale, "/private-shuttle/checkout"))}
        ...
        <div className="rounded-3xl border border-sunset-orange/20 bg-sunset-orange/5 p-6 sm:p-8">
          <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <div className="text-sm text-foreground/60">{t.thisShuttle}</div>
            ...
            >
              {submitting ? t.adding : t.addToTrip}

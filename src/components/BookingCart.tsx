    ...
    <div className="rounded-3xl border border-sunset-orange/20 bg-white p-6 shadow-sm sm:p-8">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-lg font-bold text-foreground">
          {items.length === 1 ? t.yourShuttle : t.yourShuttles}
        </h2>
      </div>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-0.5 text-xs text-foreground/50">
                {formatDate(item.date, locale)} · {formatTime(item.time, locale)} · {item.vehicleName}
              </div>
            </div>
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <div className="text-sm text-foreground/60">{t.total}</div>
          <div className="text-2xl font-bold text-foreground">${total}</div>
          <div className="text-xs text-foreground/40">{t.vatIncluded}</div>
        </div>
        <Link

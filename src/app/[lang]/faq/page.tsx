...
// Human-friendly label for a database category slug, in the page language.
function categoryLabel(key: string, locale: Locale): string {
  const labels: Record<string, string> = FAQ[locale].categories;
  return (
    labels[key] ??
    key
      .split("_")
 ...
 */
async function getFaqs(locale: Locale): Promise<Faq[]> {
  ...
  return (data ?? []).map((row) => normalise(row, locale));
}
...
function FaqJsonLd({ faqs, locale }: { faqs: Faq[]; locale: Locale }) {
  if (faqs.length === 0) return null;
  const t = FAQ[locale].jsonLd;

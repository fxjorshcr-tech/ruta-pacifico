...
function PairRow({ pair, locale }: { pair: RoutePair; locale: Locale }) {
  const t = PRICES[locale].table;
  const { out, back } = pair;
  const href = `/private-shuttle/${routeSlug(out.origen, out.destino)}`;
  const backHref = back ? `/private-shuttle/${routeSlug(back.origen, back.destino)}` : null;
  return (
    <>
      <tr className="transition hover:bg-sunset-orange/5">
        <th scope="row" className="px-4 py-3 font-medium text-foreground">
          <Link href={href} className="hover:text-sunset-orange">
            {out.origen}
            <span className="mx-1.5 text-foreground/30">{back ? "→" : "↔"}</span>
            {out.destino}
          </Link>
        </th>
        <td className="px-4 py-3 text-foreground/70 whitespace-nowrap">{travelTime(out, locale)}</td>
        {VEHICLE_TIERS.map((tier) => (
          <td key={tier.key} className="px-4 py-3 text-right whitespace-nowrap">
            <PriceCell route={out} field={tier.priceField} onRequest={t.onRequest} />
          </td>
        ))}
      ...
      {back && backHref ? (
        <tr className="bg-light-surface/60 transition hover:bg-sunset-orange/5">
          <th scope="row" className="px-4 py-2 pl-8 text-xs font-medium text-foreground/70">
            <Link href={backHref} className="hover:text-sunset-orange">
              {t.returnLabel}: {back.origen}
              <span className="mx-1.5 text-foreground/30">→</span>
              {back.destino}
            </Link>
          </th>
          <td className="px-4 py-2 text-xs text-foreground/60 whitespace-nowrap">{travelTime(back, locale)}</td>
          {VEHICLE_TIERS.map((tier) => (
            <td key={tier.key} className="px-4 py-2 text-right text-xs whitespace-nowrap">
              <PriceCell route={back} field={tier.priceField} onRequest={t.onRequest} />
            </td>
          ))}
          <td className="px-4 py-2 text-right">
            <Link href={backHref} className="text-xs font-semibold text-foreground/60 hover:text-sunset-orange">
              {t.book}
            </Link>
          </td>
        </tr>
...
function GroupTable({ group, locale, defaultOpen }: { group: PairGroup; locale: Locale; defaultOpen: boolean }) {
  const t = PRICES[locale].table;
  const tp = PRICES[locale];
  const id = `prices-${group.key}`;
  return (
    // Native <details>: only the first group is open by default so the page
    // is not a wall of rows, yet every route stays in the DOM for crawlers
    // and answer engines (collapsed content is indexed normally). Browsers
      ...
      >
        ...
        <span className="flex shrink-0 items-center gap-3">
          <span className="hidden rounded-full bg-light-surface px-3 py-1 text-xs font-semibold text-foreground/60 sm:inline">
            {tp.jumpNav.count(group.pairs.length)}
          </span>
          <svg
        ...
        <table className="w-full min-w-[640px] text-left text-sm">
          ...
          <tbody className="divide-y divide-black/5">
            {group.pairs.map((pair) => (
              <PairRow key={pair.out.id} pair={pair} locale={locale} />
            ))}
          </tbody>
        </table>
      </div>
...
export default async function PricesPage({ params }: { params: Params }) {
  ...
  const pairGroups = pairRoutesForPriceList(routes, { indexableOnly: true, destinations, locale });
  const all = groups.flatMap((g) => g.routes);
  const lir = groups.find((g) => g.key === "from-lir");
  // The hero quotes the cheapest airport transfer from Liberia, which is what
  // almost every visitor is here for; the cheapest route overall is a $75
  // SJO hop to downtown San José and only confused people.
  const range = priceRange(lir?.routes ?? all);
  ...
  return (
    <main className="bg-light-surface min-h-screen">
          ...
          {pairGroups.length > 1 ? (
            <nav aria-label={t.jumpNav.label} className="mt-8 flex flex-wrap gap-2 border-t border-black/5 pt-6">
              {pairGroups.map((g) => (
                ...
                >
                  {t.jumpNav.item(g.title, g.pairs.length)}
                </a>
              ))}
            </nav>
          ) : null}
      ...
      <div className="mx-auto max-w-5xl space-y-6 px-6 py-16">
        {all.length ? (
          pairGroups.map((g, i) => <GroupTable key={g.key} group={g} locale={locale} defaultOpen={i === 0} />)
        ) : (

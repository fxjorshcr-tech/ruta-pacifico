...
import { CONFIRMATION } from "@/i18n/checkout";
import {
  BOOKING_STORAGE_KEY,
  formatDate,
  formatTime,
...
/** Icon + colour per trust badge; the text comes from CONFIRMATION[locale].badges in the same order. */
const BADGE_STYLES = [
  { color: "text-blue-500 bg-blue-50", icon: "M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9-1.5H3.375c-.621 0-1.125-.504-1.125-1.125V9.375c0-.621.504-1.125 1.125-1.125h9A1.125 1.125 0 0 1 13.5 9.375v7.5c0 .621-.504 1.125-1.125 1.125H9.75m0 0h4.5m-4.5 0a1.5 1.5 0 0 0-3 0m3 0a1.5 1.5 0 0 1-3 0m12 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 0 0-3.213-9.193 2.056 2.056 0 0 0-1.58-.86H13.5" },
  { color: "text-emerald-500 bg-emerald-50", icon: "M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" },
  { color: "text-green-600 bg-green-50", icon: "M9 12.75 11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 0 1-1.043 3.296 3.745 3.745 0 0 1-3.296 1.043A3.745 3.745 0 0 1 12 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 0 1-3.296-1.043 3.745 3.745 0 0 1-1.043-3.296A3.745 3.745 0 0 1 3 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 0 1 1.043-3.296 3.746 3.746 0 0 1 3.296-1.043A3.746 3.746 0 0 1 12 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 0 1 3.296 1.043 3.746 3.746 0 0 1 1.043 3.296A3.745 3.745 0 0 1 21 12Z" },
  { color: "text-sunset-orange bg-sunset-orange/10", icon: "M20.25 8.511c.884.284 1.5 1.128 1.5 2.097v4.286c0 1.136-.847 2.1-1.98 2.193-.34.027-.68.052-1.02.072v3.091l-3-3c-1.354 0-2.694-.055-4.02-.163a2.115 2.115 0 0 1-.825-.242m9.345-8.334a2.126 2.126 0 0 0-.476-.095 48.64 48.64 0 0 0-8.048 0c-1.131.094-1.976 1.057-1.976 2.192v4.286c0 .837.46 1.58 1.155 1.951m9.345-8.334V6.637c0-1.621-1.152-3.026-2.76-3.235A48.455 48.455 0 0 0 11.25 3c-2.115 0-4.198.137-6.24.402-1.608.209-2.76 1.614-2.76 3.235v6.226c0 1.621 1.152 3.026 2.76 3.235.577.075 1.157.14 1.74.194V21l4.155-4.155" },
];

/** Icons for the "What's included" list, same order as CONFIRMATION[locale].included. */
const INCLUDED_ICONS = [
  "M8.25 18.75a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m3 0h6m-9-1.5H3.375c-.621 0-1.125-.504-1.125-1.125V9.375c0-.621.504-1.125 1.125-1.125h9A1.125 1.125 0 0 1 13.5 9.375v7.5c0 .621-.504 1.125-1.125 1.125H9.75",
  ...
  "M8.288 15.038a5.25 5.25 0 0 1 7.424 0M5.106 11.856c3.807-3.808 9.98-3.808 13.788 0M1.924 8.674c5.565-5.565 14.587-5.565 20.152 0",
...
/** Icons for the "Important information" list, same order as CONFIRMATION[locale].important. */
...
export default function ConfirmationPage() {
  ...
  const whatsappText = t.whatsappText(
    ...
    booking.trips.map((trip) => `${trip.from} → ${trip.to}`).join(", "),
  );
  ...
  return (
    <main className="min-h-screen bg-light-surface">
      ...
      <section className="relative flex min-h-[45vh] items-center overflow-hidden">
        <Image src={HERO_URL} alt="" fill className="object-cover" sizes="100vw" preload />
        ...
        <div className="relative z-10 mx-auto w-full max-w-3xl px-6 pt-24 pb-20 text-center">
          ...
          <div className="mt-5 inline-flex items-center gap-2 rounded-full bg-white/10 px-5 py-2.5 text-sm backdrop-blur-sm border border-white/10">
            <span className="text-white/60">{t.codeLabel}</span>
      ...
      <div className="mx-auto max-w-3xl px-6 pb-20 pt-10">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-relaxed text-foreground/70">
                {t.payment.p1}
                <strong className="text-foreground">{t.payment.strong1}</strong>
                {t.payment.p2}
                <strong className="text-foreground">{t.payment.strong2}</strong>
                {t.payment.p3}
                <strong className="text-foreground">{booking.email}</strong>
                {t.payment.p4}
                <strong className="text-foreground">{booking.phone}</strong>
                {t.payment.p5}
              </p>
            </div>
          </div>
        ...
        <div className="mt-8 rounded-3xl border border-black/5 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-lg font-bold text-foreground">
            {t.shuttles(booking.trips.length)}
          </h2>
              ...
              return (
                <div key={trip.id} className="rounded-2xl border border-black/5 bg-light-surface p-5">
                  ...
                  <div className="mt-3 grid gap-2 text-xs text-foreground/60 sm:grid-cols-2">
                    <div>
                      <span className="text-foreground/40">{t.labels.date} </span>
                      {t.dateAt(formatDate(trip.date), formatTime(trip.time))}
                    </div>
                    <div>
                      <span className="text-foreground/40">{t.labels.vehicle} </span>
                      {trip.vehicleName} ({trip.vehiclePax})
                    </div>
                    <div>
                      <span className="text-foreground/40">{t.labels.travelers} </span>
                      {pax}
                    </div>
                    {trip.flight && (
                      <div>
                        <span className="text-foreground/40">{t.labels.flight} </span>
                        {trip.flight}
                      </div>
                    )}
                    <div>
                      <span className="text-foreground/40">{t.labels.pickup} </span>
                      {trip.pickup}
                    </div>
                    <div>
                      <span className="text-foreground/40">{t.labels.dropoff} </span>
                      {trip.dropoff}
                    </div>
                  </div>
                </div>
          ...
          {booking.notes && (
            <div className="mt-4 rounded-2xl border border-black/5 bg-light-surface p-4">
              <div className="text-xs font-semibold uppercase tracking-wider text-foreground/40">{t.notesTitle}</div>
          ...
          <div className="mt-6 flex items-center justify-between rounded-2xl bg-foreground p-5">
            <div>
              <div className="text-sm text-white/60">
                {t.total(booking.trips.length)}
              </div>
              <div className="text-xs text-white/40">{t.vat}</div>
            </div>
        ...
        <div className="mt-6 rounded-3xl border border-black/5 bg-white p-6 shadow-sm sm:p-8">
          <h2 className="text-sm font-bold uppercase tracking-wider text-foreground/40">{t.contact.title}</h2>
          <div className="mt-3 grid gap-2 text-sm sm:grid-cols-3">
            <div>
              <span className="text-foreground/50">{t.contact.name} </span>
              <span className="font-medium text-foreground">{booking.name}</span>
            </div>
            <div>
              <span className="text-foreground/50">{t.contact.email} </span>
              <span className="font-medium text-foreground break-all">{booking.email}</span>
            </div>
            <div>
              <span className="text-foreground/50">{t.contact.phone} </span>
        ...
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
          {t.badges.map((badge, i) => (
            <div key={badge.label} className="rounded-2xl border border-black/5 bg-white p-4 text-center shadow-sm">
              <div className={`mx-auto flex h-11 w-11 items-center justify-center rounded-full ${BADGE_STYLES[i].color}`}>
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" d={BADGE_STYLES[i].icon} />
                </svg>
              </div>
              ...
              {t.included.map((text, i) => (
                <li key={text} className="flex items-center gap-3 text-sm text-foreground/70">
                  <svg className="h-4 w-4 shrink-0 text-sunset-orange" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" d={INCLUDED_ICONS[i]} />
                  </svg>
                  {text}
                </li>
              ))}
